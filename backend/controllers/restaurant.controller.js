...
exports.getProfile = async (req, res, next) => {
  try {
    const restaurant = await Restaurant.findById(req.auth.id).select(
      "-password",
    );
    ...
    if (restaurant.isBlocked)
      return res
        ...
        .json({
          success: false,
          message: `Account blocked: ${restaurant.blockReason}`,
        });
...
exports.uploadImage = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, image: restaurant.image });
  } catch (err) {
    next(err);
  }
};
