...
const STATUS_FLOW = [
  ...
  { key: "accepted", label: "Accepted", desc: "Restaurant confirmed", icon: CheckCircle2 },
  { key: "preparing", label: "Preparing", desc: "Chef is cooking", icon: Clock },
  { key: "out for delivery", label: "On the way", desc: "Driver is nearby", icon: Truck },
...
const TrackOrder = () => {
  ...
  return (
    <div className="bg-[#F8F9FB] text-slate-900 font-sans antialiased pb-20">
      {/* Top Glass Navigation */}
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <button onClick={() => navigate("/orders")} className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-amber-600 transition-all active:scale-95 group hover:cursor-pointer">
            <ChevronLeft size={18} className="group-hover:-translate-x-1 transition-transform" /> Back to History
          </button>
          <div className="text-right">
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Order ID</p>
            <p className="text-sm font-black text-slate-800">#{orderId.slice(-8).toUpperCase()}</p>
          </div>
        </div>
      </nav>
            ...
            <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100">
              <div className="mb-12">
                <h1 className="text-2xl font-black tracking-tight mb-2">
                  {order.status === 'completed' ? 'Delivered!' : 'Coming your way'}
                </h1>
                <p className="text-slate-500 font-medium">Estimated arrival in <span className="text-amber-600">12-15 mins</span></p>
              </div>
                    ...
                    return (
                      <div key={step.key} className="flex flex-col items-center">
                        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center z-10 transition-all duration-500 ${
                          active ? 'bg-amber-500 text-white shadow-lg shadow-amber-200' : 'bg-white border-2 border-slate-100 text-slate-300'
                        } ${isCurrent ? 'ring-4 ring-amber-100 scale-110' : ''}`}>
                          <step.icon size={18} />
                        </div>
                        <div className={`absolute top-14 text-center transition-opacity duration-300 ${isCurrent ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
              <div className="flex items-center gap-2 mb-8">
                ...
                <h3 className="text-sm font-black uppercase tracking-widest">Receipt Summary</h3>
              </div>
              
              <div className="space-y-4 mb-8">
                {order.items.map((item) => (
                  <div key={item.menuItem} className="flex justify-between items-center group">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-slate-50 rounded-lg flex items-center justify-center text-[10px] font-black text-slate-400">{item.quantity}x</div>
